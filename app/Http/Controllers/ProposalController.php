...
class ProposalController extends Controller
{
    ...
    public function convertToInvoice($proposal_id)
    {
    ...
    public function convertToDeliverNote($proposal_id)
    {
        if (\Auth::user()->can('convert delivery note')) {
            $proposal = Proposal::where('id', $proposal_id)->first();
            $proposal->is_convert = 1;
            $proposal->save();

            $convertDeliveryNote = new DeliveryNote();
            $convertDeliveryNote->invoice_id = $this->deliveryNoteNumber();
            $convertDeliveryNote->customer_id = $proposal['customer_id'];
            $convertDeliveryNote->issue_date = date('Y-m-d');
            $convertDeliveryNote->due_date = date('Y-m-d');
            $convertDeliveryNote->send_date = null;
            $convertDeliveryNote->category_id = $proposal['category_id'];
            $convertDeliveryNote->status = 0;
            $convertDeliveryNote->created_by = $proposal['created_by'];
            $convertDeliveryNote->save();

            $proposal->converted_delivery_note_id = $convertDeliveryNote->id;
            $proposal->save();

            if ($convertDeliveryNote) {
                $proposalProduct = ProposalProduct::where('proposal_id', $proposal_id)->get();
                foreach ($proposalProduct as $product) {
                    $duplicateProduct = new DeliveryNoteProduct();
                    $duplicateProduct->invoice_id = $convertDeliveryNote->id;
                    $duplicateProduct->product_id = $product->product_id;
                    $duplicateProduct->quantity = $product->quantity;
                    $duplicateProduct->tax = $product->tax;
                    $duplicateProduct->discount = $product->discount;
            ...
            return redirect()->back()->with('success', __('Proposal to Delivery Note convert successfully.'));
        } else {
            return redirect()->back()->with('error', __('Permission denied.'));
        }
    }
