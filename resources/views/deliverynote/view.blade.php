...
@push('script-page')
    ...
    <script type="text/javascript">
        @if($deliverNote->getDue() > 0 &&  Utility::getValByName('enable_stripe') == 'on' && !empty(Utility::getValByName('stripe_key')) && !empty(Utility::getValByName('stripe_secret')))
...
@section('content')
    @can('send delivery note')
        @if($deliverNote->status!=4)
            <div class="row">
                <div class="col-12">
                    <div class="card mb-3">
                        <div class="card-body">
                            <div class="timeline timeline-one-side" data-timeline-content="axis"
                                ...
                                <div class="timeline-block">
                                    ...
                                    <div class="timeline-content">
                                        <div class="text-sm h6">{{__('Delivery Note Create')}}</div>
                                        @can('edit invoice')
                                            <div class="Action">
                                                <a href="{{ route('delivery-note.edit',\Crypt::encrypt($deliverNote->id)) }}"
                                        ...
                                        @endcan
                                        <small><i
                                                class="fas fa-clock mr-1"></i>{{__('Created on ')}} {{$user->dateFormat($deliverNote->issue_date)}}
                                        </small>
                                    </div>
                                </div>
                                <div class="timeline-block">
                                    ...
                                    <div class="timeline-content">
                                        <div class="text-sm h6 ">{{__('Send Delivery Note')}}</div>
                                        @if($deliverNote->status==0)
                                            <div class="Action">
                                                @can('send invoice')
                                                    <a href="{{ route('delivery-note.sent',$deliverNote->id) }}"
                                        ...
                                        @if($deliverNote->status!=0)
                                            <small><i
                                                    class="fas fa-clock mr-1"></i>{{__('Sent on')}} {{$user->dateFormat($deliverNote->send_date)}}
                                            </small>
                                        @else
                                            @can('send delivery note')
                                ...
                                <div class="timeline-block">
                                    ...
                                    <div class="timeline-content">
                                        ...
                                        @if($deliverNote->status!=0)
                                            @can('create payment delivery note')
                                                <a href="#" data-url="{{ route('delivery-note.payment',$deliverNote->id) }}"
                                                   data-ajax-popup="true" data-title="{{__('Add Receipt')}}"
                                                   class="edit-icon float-right" data-toggle="tooltip"
                                                   data-original-title="{{__('Add Receipt')}}"><i
        ...
        @if($deliverNote->status!=0)
            <div class="row justify-content-between align-items-center mb-3">
                <div class="col-md-12 d-flex align-items-center justify-content-between justify-content-md-end">
                    ...
                    @if($deliverNote->status!=4)
                        <div class="all-button-box mx-2">
                            <a href="{{ route('delivery-note.payment.reminder',$deliverNote->id)}}"
                               class="btn btn-xs btn-white btn-icon-only width-auto">{{__('Receipt Reminder')}}</a>
                        </div>
                    @endif
                    <div class="all-button-box mx-2">
                        <a href="{{ route('delivery-note.resent',$deliverNote->id)}}"
                           class="btn btn-xs btn-white btn-icon-only width-auto">{{__('Resend Delvery Note')}}</a>
                    </div>
                    <div class="all-button-box">
                        <a href="{{ route('delivery-note.pdf', Crypt::encrypt($deliverNote->id))}}" target="_blank"
            ...
            <div class="col-md-12 d-flex align-items-center justify-content-between justify-content-md-end">
                <div class="all-button-box mx-2">
                    <a href="#" class="btn btn-xs btn-white btn-icon-only width-auto"
                       data-url="{{route('customer.invoice.send',$deliverNote->id)}}" data-ajax-popup="true"
                       data-title="{{__('Send Delivery Note')}}">
                        {{__('Send Mail')}}
                    </a>
                </div>
                <div class="all-button-box mx-2">
                ...
                @if($deliverNote->status!=0 && $deliverNote->getDue() > 0 && ((Utility::getValByName('enable_stripe') == 'on' && !empty(Utility::getValByName('stripe_key')) && !empty(Utility::getValByName('stripe_secret'))) || (Utility::getValByName('enable_paypal')== 'on' && !empty(Utility::getValByName('paypal_client_id')) && !empty(Utility::getValByName('paypal_secret_key')))))
                    <div class="all-button-box">
                        ...
                        <div class="invoice-print">
                            <div class="row invoice-title mt-2">
                                <div class="col-xs-12 col-sm-12 col-nd-6 col-lg-6 col-12">
                                    <h2>{{ $user->deliveryNoteNumberFormat($deliverNote->invoice_id) }}</h2>
                                </div>
                                ...
                                <div class="col-xs-12 col-sm-12 col-nd-6 col-lg-6 col-12 text-right">
                                    ...
                                    <div class="float-right ml-2">
                                        {!! DNS2D::getBarcodeHTML(route('invoice.link.copy',\Illuminate\Support\Facades\Crypt::encrypt($deliverNote->id)), "QRCODE",2,2) !!}
                                    </div>
                                    <!-- QR code module end-->
                                ...
                                @if(\Utility::getValByName('shipping_display')=='on')
                                    <div class="col-md-6 text-md-right">
                                        <small>
                                            ...
                                            {{!empty($customer->shipping_city)?$customer->shipping_city:'' . ', '}} {{!empty($customer->shipping_state)?$customer->shipping_state:'' .', '}}
                            ...
                            <div class="row mt-3">
                                <div class="col">
                                    <small>
                                        ...
                                        @if($deliverNote->status == 0)
                                            <span
                                                class="badge badge-pill badge-primary">{{ __(\App\Models\DeliveryNote::$statues[$deliverNote->status]) }}</span>
                                        @elseif($deliverNote->status == 1)
                                            <span
                                                class="badge badge-pill badge-warning">{{ __(\App\Models\DeliveryNote::$statues[$deliverNote->status]) }}</span>
                                        @elseif($deliverNote->status == 2)
                                            <span
                                                class="badge badge-pill badge-danger">{{ __(\App\Models\DeliveryNote::$statues[$deliverNote->status]) }}</span>
                                        @elseif($deliverNote->status == 3)
                                            <span
                                                class="badge badge-pill badge-info">{{ __(\App\Models\DeliveryNote::$statues[$deliverNote->status]) }}</span>
                                        @elseif($deliverNote->status == 4)
                                            <span
                                                class="badge badge-pill badge-success">{{ __(\App\Models\DeliveryNote::$statues[$deliverNote->status]) }}</span>
                                        @endif
                                    </small>
                                </div>
                                <div class="col text-md-center">
                                    <small>
                                        ...
                                        {{$user->dateFormat($deliverNote->issue_date)}}<br><br>
                                    </small>
                                </div>
                                <div class="col text-md-right">
                                    <small>
                                        <strong>{{__('Due Date')}} :</strong><br>
                                        {{$user->dateFormat($deliverNote->due_date)}}<br><br>
                                    </small>
                                </div>

                                @if(!empty($customFields) && count($deliverNote->customField)>0)
                                    @foreach($customFields as $field)
                                        <div class="col text-md-right">
                                            <small>
                                                <strong>{{$field->name}} :</strong><br>
                                                {{!empty($deliverNote->customField)?$deliverNote->customField[$field->id]:'-'}}
                                                <br><br>
                                            </small>
                                        </div>
                                    @endforeach
                                    ...
                                    <div class="table-responsive mt-2">
                                        <table class="table mb-0 table-striped">
                                            <tr>
                                                ...
                                                <th class="text-dark">@if($deliverNote->discount_apply==1)
                                                        {{__('Discount')}}
                                                    @endif</th>
                                            ...
                                            @foreach($iteams as $key =>$iteam)
                                                ...
                                                <tr>
                                                    ...
                                                    <td> @if($deliverNote->discount_apply==1)
                                            ...
                                            <tr>
                                                ...
                                                <td class="text-right">{{$user->priceFormat($deliverNote->getSubTotal())}}</td>
                                            </tr>
                                            @if($deliverNote->discount_apply==1)
                                                <tr>
                                                    <td colspan="6"></td>
                                                    <td class="text-right"><b>{{__('Discount')}}</b></td>
                                                    <td class="text-right">{{$user->priceFormat($deliverNote->getTotalDiscount())}}</td>
                                                </tr>
                                            @endif
                                            @if(!empty($taxesData))
                                            ...
                                            <tr>
                                                ...
                                                <td class="blue-text text-right">{{$user->priceFormat($deliverNote->getTotal())}}</td>
                                            </tr>
                                            <tr>
                                                <td colspan="6"></td>
                                                <td class="text-right"><b>{{__('Paid')}}</b></td>
                                                <td class="text-right">{{$user->priceFormat(($deliverNote->getTotal()-$deliverNote->getDue())-($deliverNote->deliveryNoteTotalCreditNote()))}}</td>
                                            </tr>
                                            <tr>
                                                <td colspan="6"></td>
                                                <td class="text-right"><b>{{__('Credit Note')}}</b></td>
                                                <td class="text-right">{{$user->priceFormat(($deliverNote->deliveryNoteTotalCreditNote()))}}</td>
                                            </tr>
                                            <tr>
                                                <td colspan="6"></td>
                                                <td class="text-right"><b>{{__('Due')}}</b></td>
                                                <td class="text-right">{{$user->priceFormat($deliverNote->getDue())}}</td>
                                            </tr>
                                            </tfoot>
                                        </table>
                                    </div>
                ...
                <div class="card-body py-0">
                    <div class="table-responsive">
                        <table class="table table-striped">
                            ...
                            @forelse($deliverNote->payments as $key =>$payment)
                                <tr>
                                    <td>
                                    ...
                                    <td>@if(!empty($payment->receipt))
                                            <a href="{{$payment->receipt}}" target="_blank"> <i class="fas fa-file"></i></a>
                                        @else
                                            --
                                        @endif</td>
                                    ...
                                    @can('delete invoice product')
                                        <td>
                                            ...
                                            {!! Form::open(['method' => 'post', 'route' => ['delivery-note.payment.destroy',$deliverNote->id,$payment->id],'id'=>'delete-form-'.$payment->id]) !!}
                                            {!! Form::close() !!}
                                        </td>
                                    @endcan
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="{{ (Gate::check('delete invoice product') ? '9' : '8') }}"
                                        class="text-center text-dark"><p>{{__('No Data Found')}}</p></td>
                                </tr>
                            @endforelse
                        </table>
                    </div>
                ...
                <div class="card-body py-0">
                    <div class="table-responsive">
                        <table class="table table-striped">
                            ...
                            @if(is_array($deliverNote->creditNote) || is_object($deliverNote->creditNote))
                                @forelse($deliverNote->creditNote as $key =>$creditNote)
                                    <tr>
                            ...
                            <div class="card bg-none card-box">
                                <section class="nav-tabs p-2">
                                                ...
                                                @if(Utility::getValByName('enable_stripe') == 'on')
                                                    <li>
                                                        <a data-toggle="tab" class="active" id="contact-tab4"
                                                           href="#stripe_payment">{{__('Stripe')}}</a>
                                                    </li>
                                                @endif
                                                @if(Utility::getValByName('enable_paypal') == 'on')
                                                    <li>
                                                        <a data-toggle="tab" id="contact-tab5"
                                                           href="#paypal_payment">{{__('Paypal')}}</a>
                                                    </li>
                                                @endif
                                            </ul>
                                        </div>
                                    ...
                                    <div class="tab-content">
                                        @if(Utility::getValByName('enable_stripe') == 'on')
                                            <div
                                                class="tab-pane {{ ((Utility::getValByName('enable_stripe') == 'on' && Utility::getValByName('enable_paypal') == 'on') || Utility::getValByName('enable_stripe') == 'on') ? "show active" : "" }}"
                                                id="stripe_payment">
                                                <form method="post"
                                                    ...
                                                    <div class="row">
                                                        ...
                                                        <div class="col-sm-4 text-sm-right mt-3 mt-sm-0">
                                                            ...
                                                            <img
                                                                src="{{asset('assets/img/payments/american express.png')}}"
                                                        ...
                                                        <div class="form-group col-md-12">
                                                            ...
                                                            <div class="input-group">
                                                                <input class="form-control" required="required" min="0"
                                                                       ...
                                                                       value="{{$deliverNote->getDue()}}" min="0"
                                                                       step="0.01" max="{{$deliverNote->getDue()}}"
                                                                       id="amount">
                                                            </div>
                                                        </div>
                                                    </div>
                                                    <div class="row">
                                                        <div class="col-12">
                                                            <div class="error" style="display: none;">
                                                                <div
                                                                    class='alert-danger alert'>{{__('Please correct the errors and try again.')}}</div>
                                                            </div>
                                                        </div>
                                                    </div>
                                        ...
                                        @if(Utility::getValByName('enable_stripe') == 'on')
                                            <div
                                                class="tab-pane fade {{ (Utility::getValByName('enable_stripe') != 'on' && Utility::getValByName('enable_paypal') == 'on') ? "active" : "" }}"
                                                id="paypal_payment">
                                                <form method="post"
                                                      action="{{ route('customer.pay.with.paypal',$deliverNote->id) }}"
                                                      class="require-validation">
                                                    @csrf
                                                    <div class="row">
                                                        <div class="col-sm-8">
